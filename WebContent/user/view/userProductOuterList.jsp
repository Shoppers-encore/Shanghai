...
<html>
	...
	<body>
		...
		<div class="jumbotron col-12 bg-white text-center productCategoryJumbotron mb-0">
			<h5>OUTER</h5>		
			...
			<small><a class="col-3 text-secondary" href="userProductOuterList.jk?category=CARDIGAN">${str_cat_CARDIGAN}</a></small>
		</div>
				...
				<div class="text-center">
					<c:if test="${productCount gt 0}">
						<c:if test="${currentPage ne 1}">
							<a href="userProductOuterList.jk?category=${category}">
								<img src="/urPresent/images/rewind.png" class="rewindAndFastForward">
							</a>
							<c:if test="${startPage gt pageBlock}">
								<a href="userProductOuterList.jk?category=${category}&pageNum=${startPage-pageBlock}">
						...
						<c:forEach var="i" begin="${startPage}" end="${endPage}">
							...
							<c:if test="${i ne currentPage}">
								<a href="userProductOuterList.jk?category=${category}&pageNum=${i}"><strong>${i}</strong></a>
							</c:if>
						</c:forEach>
						<c:if test="${currentPage ne pageCount}">
							...
							<a href="userProductOuterList.jk?category=${category}&pageNum=${pageCount}">
